...
namespace League.Services
{
    public class ChatMessageBuilder
    {
        ...
        // ==================== 对外调用方法 ====================
        public string BuildMyTeamSummary(JArray? team)
        {
            return BuildTeamSummary(team, detailed: false);
        }

        public string BuildFullTeamSummary(JArray? myTeam, JArray? enemyTeam)
        {
            var sb = new StringBuilder();
            string my = BuildTeamSummary(myTeam, detailed: false);
            if (!string.IsNullOrWhiteSpace(my))
            {
                sb.AppendLine(my);
                sb.AppendLine();
            ...
            if (enemyTeam?.Count > 0 == true)
            {
                string enemy = BuildTeamSummary(enemyTeam, detailed: true);
                if (!string.IsNullOrWhiteSpace(enemy))
                    sb.AppendLine(enemy);
            }
            return sb.ToString().TrimEnd();
        ...
        // 内部统一方法
        private string BuildTeamSummary(JArray? team, bool detailed)
        {
            if (team == null || team.Count == 0)
                return "暂无数据";
            ...
            foreach (var p in team)
            {
                ...
                string line = detailed
                    ? BuildDetailedLine(info, name)
                    : BuildSimpleLine(info, name);
        ...
        // ==================== 简略版（F7 和 F9） - 已增强显示信号详情 ====================
        private string BuildSimpleLine(PlayerMatchInfo info, string playerName)
        {
            int gameCount = info.WinHistory?.Count ?? 0;
            if (gameCount == 0)
                return $"{playerName} 暂无战绩";
            ...
            if (string.IsNullOrEmpty(behavior))
            {
                return $"{playerName} {gameText} 胜率 [{winRate:F0}%]{detailStr}";
            }
            else
            {
                return $"{playerName} {gameText} 胜率 [{winRate:F0}%]{detailStr} 特征 {behavior}";
            }
        }

        // ==================== 详细版（F11） ====================
        private string BuildDetailedLine(PlayerMatchInfo info, string playerName)
        {
            int gameCount = info.WinHistory?.Count ?? 0;
            if (gameCount == 0)
                return $"{playerName} 暂无战绩";
