...
namespace League
{
    public partial class FormMain : Form
    {
        ...
        /// 监听热键并发送消息
        /// </summary>
        public void ListenAndSendMessageWhenHotkeyPressed(JArray myTeam)
        {
            _messageSender!.ListenAndSendMessageWhenHotkeyPressed(myTeam, SendChampSelectSummaryViaSendKeys);
        }

        /// <summary>
        /// 发送我方队伍数据到选人聊天窗口（保持原有逻辑）
        /// </summary>
        private void SendChampSelectSummaryViaSendKeys(JArray myTeam)
        {
            ...
            foreach (var p in myTeam)
            {
                ...
                if (summonerId == 0 || !_cachedPlayerMatchInfos.TryGetValue(summonerId, out var info))
                {
                    continue;
                }
