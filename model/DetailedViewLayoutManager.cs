...
//                BackColor = backColor
//            };
//            await PreloadAllAsync(participant);
...
//            panel.SuspendLayout();
//            CreateAllUI(panel, participant, playerName, isSelf, ranks);
...
//        private void CreateAllUI(Panel panel, JObject participant, string playerName, bool isSelf, (int damage, int damageRank, int gold, int goldRank, double kp, int kpRank, int damageTaken, int damageTakenRank) ranks)
//        {
//            CreateHeroImage(panel, participant);
//            CreateFirstRowStats(panel, participant, playerName, isSelf, ranks);
//            CreateSecondRowItems(panel, participant);
//        }
...
//        {
//            Label nameLabel = CreateLabel(playerName, 80, 15, 200, isSelf ? FontStyle.Bold : FontStyle.Regular, isSelf ? Color.Blue : Color.Black, 10f);
//            panel.Controls.Add(nameLabel);
//            panel.Controls.Add(CreateSeparator(290, 15));
//            Label kdaLabel = CreateLabel($"数据: {participant["kills"]}/{participant["deaths"]}/{participant["assists"]}", 305, 15, 130, FontStyle.Bold, Color.DeepSkyBlue, 10f);
//            panel.Controls.Add(kdaLabel);
//            panel.Controls.Add(CreateSeparator(445, 15));
//            Label damageLabel = CreateLabel($"伤害: {ranks.damage} (#{ranks.damageRank})", 460, 15, 140, FontStyle.Regular, Color.OrangeRed, 10f);
//            panel.Controls.Add(damageLabel);
//            panel.Controls.Add(CreateSeparator(590, 15));
//            Label goldLabel = CreateLabel($"经济: {ranks.gold} (#{ranks.goldRank})", 615, 15, 130, FontStyle.Regular, Color.Goldenrod, 10f);
//            panel.Controls.Add(goldLabel);
//            panel.Controls.Add(CreateSeparator(735, 15));
//            Label csLabel = CreateLabel($"补刀: {participant["totalMinionsKilled"]}", 760, 15, 90, FontStyle.Regular, Color.DarkGreen, 10f);
//            panel.Controls.Add(csLabel);
//            panel.Controls.Add(CreateSeparator(850, 15));
//            Label kpLabel = CreateLabel($"参团: {ranks.kp * 100.0:F1}% (#{ranks.kpRank})", 875, 15, 130, FontStyle.Regular, Color.DodgerBlue, 10f);
//            panel.Controls.Add(kpLabel);
//            Label damageTakenLabel = CreateLabel($"承伤: {ranks.damageTaken} (#{ranks.Rest.Item1})", 1010, 15, 130, FontStyle.Regular, Color.MediumPurple, 10f);
...
//                else
//                {
//                    CreatePlaceholderImage(panel, 15, 25, 50, 50, "英雄");
//                }
//            }
//        }

//        private void CreateSecondRowItems(Panel panel, JObject participant)
//        {
//            Label spellsLabel = CreateLabel("召唤师技能:", 80, 60, 80, FontStyle.Bold, Color.Black, 9f);
//            Label itemsLabel = CreateLabel("装备:", 250, 60, 40, FontStyle.Bold, Color.Black, 9f);
//            Label runesLabel = CreateLabel("符文:", 580, 60, 40, FontStyle.Bold, Color.Black, 9f);
//            panel.Controls.AddRange(new Control[3] { spellsLabel, itemsLabel, runesLabel });
//            panel.Controls.Add(CreateSeparator(235, 60));
//            panel.Controls.Add(CreateSeparator(505, 60));
//            CreateSpellImages(panel, participant, 165, 60);
//            CreateItemImages(panel, participant, 295, 60);
//            CreateRuneImages(panel, participant, 630, 60);
//        }
...
//        private void CreateRuneImages(Panel panel, JObject participant, int startX, int y)
//        {
//            (RuneInfo[], RuneInfo[], RuneInfo[]) runesData = LoadRunesForParticipant(participant);
//            int x = startX;
//            if (runesData.Item1 != null)
//            {
//                foreach (RuneInfo rune in runesData.Item1.Where((RuneInfo r) => r != null))
//                {
//                    if (_runeCache.TryGetValue(rune.RuneId, out (Image, string, string) info) && info.Item1 != null)
//                    {
//                        PictureBox pic = CreatePictureBox(info, x, y + 2, 30, 30, "符文: " + info.Item2);
//                        panel.Controls.Add(pic);
//                    }
//                    x += 37;
//                }
...
//            {
//                foreach (RuneInfo rune2 in runesData.Item2.Where((RuneInfo r) => r != null))
//                {
//                    if (_runeCache.TryGetValue(rune2.RuneId, out (Image, string, string) info2) && info2.Item1 != null)
//                    {
//                        PictureBox pic2 = CreatePictureBox(info2, x, y + 2, 30, 30, "符文: " + info2.Item2);
...
//                if (shard != null && _runeCache.TryGetValue(shard.RuneId, out (Image, string, string) info3) && info3.Item1 != null)
//                {
//                    PictureBox pic3 = CreatePictureBox(info3, x, y + 2, 30, 30, shardTypes[i] + "碎片: " + info3.Item2);
...
//        private async Task PreloadAllAsync(JObject participant)
//        {
...
//                return;
//            }
//            foreach (RuneInfo rune in runes.Where((RuneInfo r) => r != null && !_runeCache.ContainsKey(r.RuneId)))
//            {
...
//            return (damage: currentDamage, damageRank: GetRank(allParticipants.Select((JObject p) => p["totalDamageDealtToChampions"]?.Value<int>() ?? 0).ToList(), currentDamage), gold: currentGold, goldRank: GetRank(allParticipants.Select((JObject p) => p["goldEarned"]?.Value<int>() ?? 0).ToList(), currentGold), kp: currentKP, kpRank: GetRank(allParticipants.Select((JObject p) => (p["challenges"]?["killParticipation"]?.Value<double>()).GetValueOrDefault()).ToList(), currentKP), damageTaken: currentDamageTaken, damageTakenRank: GetRank(allParticipants.Select((JObject p) => p["totalDamageTaken"]?.Value<int>() ?? 0).ToList(), currentDamageTaken));
//        }
...
//            }
//            RuneInfo[] primaryRunes = (from r in styles.FirstOrDefault((JToken s) => s["description"]?.ToString() == "primaryStyle")?["selections"]?.Cast<JObject>().Take(4).Select(delegate (JObject selection)
//            {
...
//                                       where r != null
//                                       select r).ToArray();
//            RuneInfo[] secondaryRunes = (from r in styles.FirstOrDefault((JToken s) => s["description"]?.ToString() == "subStyle")?["selections"]?.Cast<JObject>().Take(2).Select(delegate (JObject selection)
//            {
//                int num = selection["perk"]?.Value<int>() ?? 0;
//                return (num > 0) ? new RuneInfo
//                {
...
//                }.Where((RuneInfo r) => r != null).ToArray();
//            }
...
namespace League.model
{
    public class DetailedViewLayoutManager : IDisposable
    {
        ...
        private void CreateSecondRowItems(Panel panel, JObject participant)
        {
            ...
            CreateRuneImages(panel, participant, 630, baseY);
        }
